...
class SDRTrunkDetector:
    ...
    def get_recording_directory(self) -> Optional[Path]:
        ...
        if recording_dir:
            logger.info(
                f"Found SDRTrunk recording directory from preferences: {recording_dir}"
            )
        ...
        if log_recording_dir:
            logger.info(
                f"Found SDRTrunk recording directory from logs: {log_recording_dir}"
            )
    ...
    def _get_windows_recording_dir():
        ...
        try:
            reg_path = (
                "Software\\JavaSoft\\Prefs\\io\\github\\dsheirer\\preference\\directory"
            )
    ...
    def _get_macos_recording_dir():
        ...
        try:
            ...
            sdrtrunk_plist = os.path.expanduser(
                "~/Library/Preferences/io.github.dsheirer.plist"
            )
            if os.path.exists(sdrtrunk_plist):
                ...
                if "/io/github/dsheirer/" in prefs:
                    ...
                    if (
                        "preference/" in app_prefs
                        and "directory/" in app_prefs["preference/"]
                    ):
