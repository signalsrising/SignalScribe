...
class WhisperModel:
        ...
        try:
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"Downloading {target_path.name}",
                    total=file_size
                )
    ...
    def _get_missing_files(self) -> List[Tuple[str, Dict]]:
        ...
        for file_type in self.required_files:
            ...
            if file_path.exists() and self._calculate_hash(file_path) == file_info['sha256']:
                continue
    ...
    def ensure_models_exist(self) -> bool:
        ...
        files_str = ', '.join(f"{type_}" for type_, _ in missing_files)
        logger.warning(f"Missing or invalid model files: {files_str}")
        logger.info(f"Need to download {len(missing_files)} files:")
        
        for file_type, info in missing_files:
            logger.info(f"  • {info['filename']}: {format_size(file_sizes[file_type])}")
        logger.info(f"Total download size: {format_size(total_size)}")
        
        if not Confirm.ask("Would you like to download the missing model files?"):
            return False
